...
import { useInView } from 'react-intersection-observer'; // لتفعيل الأنيميشين عند التمرير
import SvgIcon from './costumsvg';
import "../App.css";

const About = () => {
  const { ref, inView } = useInView({
    triggerOnce: false, // لتفعيل الأنيميشين كل مرة يدخل القسم في الشاشة
    threshold: 0.2, // عند وصول 20% من العنصر إلى الشاشة، سيظهر الأنيميشين
  });
  ...
  return (
    ...
    >
      ...
      <div className="flex z-20 flex-col md:flex-row items-center justify-center gap-20 w-full max-w-5xl">
        {/* Image with new animation (slide up and fade in) */}
        <motion.img
          src="/pic/inner-pages-img-16.jpg"
          alt="Restaurant Interior"
          className="w-full md:w-2/5 object-cover rounded-lg shadow-lg"
          ref={ref}
          initial={{ opacity: 0, y: 30 }} // بداية الصورة خارج الشاشة قليلا
          animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 1, ease: "easeOut" }} // تأثير دخول سلس
        />

        {/* SVG icon without animation */}
        <div
          className="w-full z-20 md:w-2/5 flex justify-center"
        >
          <SvgIcon />
        </div>

        {/* Another image with a slide effect from left */}
        <motion.img
          src="/pic/menu6.jpg"
          alt="Delicious Dish"
          className="w-full z-20 md:w-2/5 object-cover rounded-lg shadow-lg"
          ref={ref}
          initial={{ opacity: 0, x: -50 }} // بداية الصورة من الجهة اليسرى
